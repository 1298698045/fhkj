<template>
  <div class="wrap">
    ...
    <div class="siper-wrap">
      <scroll-view class="scroll-view" :scroll-left="scrollLeftIndex*itemWidth" scroll-x scroll-with-animation>
        <view class="item" :style="{'width':itemWidth+'px'}" v-for="(item,index) in dateList" :key="index"  @tap="clickDate(index)">
        <view class='text-view'>
        <view class='day' :class='index==clickIndex?"day-active":""'>{{item.day}}</view>
        <text class='month'>{{item.month}}月</text>
        <view v-if="item.month == sysmonth && item.day == nowDate">
  ...
  onLoad(){
    ...
    wx.getSystemInfo({
      success: (res) => {
        ...
        this.itemWidth = parseInt(res.windowWidth / 7);
        },
    })
    this.initData();
    this.getQuery();
  ...
  methods:{
    // 初始化日期
      initData() {
        ...
        let dateList = [];
        for (let i = -30; i < 30; i++) {
          let obj = this.getDateInfoA(nowDateTime + i * 24 * 60 * 60 * 1000);
          obj.isChoose = i == 0;
          dateList.push(obj);
        }
        this.dateList = dateList;
        this.clickIndex = 30;
        this.scrollLeftIndex = 30;
      },
      // 点击日期方法
      clickDate(index) {
        // console.log(e);
      ...
      transformationDate(date,month){
        var strDate;
        var oldDate= this.strToDate(date);
        var newDate = this.strToDate(date);
